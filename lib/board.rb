...
class Board
  ...
  def move_piece(start_pos, end_pos)
    ...
    @check_counter += 1 if scan_for_check
  end

  def set_cell(start_pos)
  ...
  def scan_for_check
    @board_cells.each do |array|
      array.each do |cell|
        unless cell == ' '
          set_cell(cell.position)
          moves = calc_moves(cell.position)
          return true if include_enemy_king?(moves)
        end
      end
    end
    false
  ...
  def include_enemy_king?(move_array)
  ...
  def filter_moves(possible_moves)
    ...
    remove_blocked_moves(possible_moves)
  end
  ...
  def blocked_move?(move)
    return true if move.any?(&:negative?) || move.any? { |n| n > 7 }

    cell = @board_cells[move[0]][move[1]]
    return false if cell == ' '

    return true if cell.color == @current_cell.color || illegal_capture(move, cell)
